Feature: Working on BCU Automation Assigment for get API for Events
...
Scenario: Verify Get API through page no should return a 200 OK code for Successfull request
	When I send a GET request to the API 'event/page/1'
	Then the response status code should be 200


Scenario: Verify Get API through page no should return a 400 status code for Bad request
	When I send a GET request to the API 'event/page/$%$%'
	Then the response status code should be 400
	#And the response should contain the event details

Scenario: Verify given a page number and sort options the API should return a paginated list of events
	When I send a GET request to the API 'event/page/0'
	Then the response status code should be 200
	And Verify the data is returned in Sorted manner

Scenario: Response of Get API through page no should include the total number of items and an array of event objects.
	When I send a GET request to the API 'event/page/1'
	Then the response status code should be 200
...
Scenario Outline: Verify Get API through page no should status code for different query prameters
	When I send a GET request to the API 'event/page/<QueryString>'
...
Scenario: Validate The Get API through page no should take less than one second to respond for a page size of 100 items.
	When I send a GET request to the API 'event/page/100'
	Then the response time should be less than 1 second
	

Scenario: Verify Get API through event ID should return a 200 OK code for Sucessfull request
	When I send a GET request to the API 'event/1'
	Then the response status code should be 200
